...
def ingresaLetraJugador(): # Permite al jugador typear que letra desea ser. # Devuelve una lista con las letras de los jugadores como primer item, y la de la computadora como segundo.
    letra = ''
    
    while not (letra == 'X' or letra == 'O'): ## mientras letra no sea x u o entonces ejecuatar esto..
...
def hacerJugada(tablero, letra, jugada): #### los parametros de esta funcion recibe la lista tablera, la letra y la posicion que ocupara esta en la lista
    tablero[jugada] = letra             ### tablero["","","","","","","","","",""] , X , 3
def esGanador(ta, le):
 # Dado un tablero y la letra de un jugador, devuelve True (verdadero) si el mismo ha ganado.
 #  reemplazamos tablero por ta y letra por le para no escribir tanto.
...
1 2 3 """) 
print("--------------")
print("Asi son las coordenas para hacer una jugada")
...
while True:
 # Resetea el tablero
    ...
    if not jugarDeNuevo():
        o_usuario = input("Ayudenos a mejorar, cuentenos que le parecio el juego: \n")
        print("Nos vemos♥")
        break
